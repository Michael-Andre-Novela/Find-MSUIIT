...
class ActivityLogView(QWidget):
	def __init__(self, parent: Optional[QWidget] = None):
		...
		self.btn_refresh.setObjectName("refreshButton")# Easy target hooks for assets/styles.qss!

		header_layout.addWidget(self.lbl_title)
		header_layout.addStretch() # Pushes the button to the far right edge
		...
		self.combo_filter.addItems(["All Activity",
							        "Items Registration",
									"Claims Processed",
									"System Alerts"
									])
		...
		filter_layout.addStretch() # Pushes the controls to the left edge

		self.main_layout.addLayout(filter_layout)

		# 4. Large monitor layout block
		self.log_display = QTextEdit(self)
		self.log_display.setReadOnly(True) # Prevents user edits to the log text
